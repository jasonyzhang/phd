...
def get_prediction_model(model_type='AZ_FC2GN'):
    models = {
        'AZ_FC2GN': az_fc2_groupnorm,
    }
    if model_type in models.keys():
        ...
        print('Unknown prediction model:', model_type)
        exit(1)
...
def az_fc2_groupnorm(net, num_conv_layers, prefix, reuse=None):
    """
    Causal architecture for movie strip encoder and autoregressive prediction.
...
def az_fc_causal_block2(net_input, num_filter, kernel_width, prefix='', name='',
                        pad_edges=False, reuse=None):
    """
    Causal res block:
    BN -> Relu -> Weight -> BN -> Relu -> Weight -> Add

    Causal is implemented by padding the left size by receptive field - 1 and
    using 'VALID' mode. The output indices thus only look at the input indices
    that are less than or equal.
    """
    pad_mode = 'EDGE' if pad_edges else 'ZERO'
    num_groups = 32
    ...
    net_norm = tf.contrib.layers.group_norm(
        ...
        channels_axis=-1,
        reduction_axes=(-3, -2),
        scope=(prefix + 'AZ_FC_causal_block_preact_gn1' + name),
        reuse=reuse,
        groups=num_groups,
    )
    # relu
    ...
    # weight
    net_relu_padded = tf_pad(
        tensor=net_relu,
        paddings=[[0, 0], [kernel_width - 1, 0], [0, 0], [0, 0]],
        mode=pad_mode
    )
    net_conv1 = tf.contrib.layers.conv2d(
        inputs=net_relu_padded,
        ...
        rate=1,
        activation_fn=None,
        scope=prefix + 'AZ_FC_causal_block2_conv1' + name,
        reuse=reuse,
    )
    # group-norm
    net_norm2 = tf.contrib.layers.group_norm(
        ...
        reuse=reuse,
        groups=num_groups,
        scope=(prefix + 'AZ_FC_causal_block_preact_gn2' + name),
    )
    # relu
    net_relu2 = tf.nn.relu(net_norm2)
    net_relu_padded2 = tf_pad(
        tensor=net_relu2,
        paddings=[[0, 0], [kernel_width - 1, 0], [0, 0], [0, 0]],
        mode=pad_mode
    )
    ...
    net_final = tf.contrib.layers.conv2d(
        ...
        activation_fn=None,
        weights_initializer=small_xavier,
        scope=prefix + 'AZ_FC_causal_block2_conv2' + name,
        reuse=reuse,
    )
    # NT1C -> NTC
